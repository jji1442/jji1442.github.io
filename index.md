...
- 노말 스테이지
	...
	- (승리 시) 몬스터의 체력이 0이 되면 보상이 주어지며, 이 보상은 가방에 누적
	- 가방을 클릭 시, 노말 스테이지에 대한 보상을 받게 되고 노말 스테이지 1단계로 초기화

- 보스 스테이지

	- (승리 시) 보스 체력을 0이 되면 보상이 주어지며, 이 보상은 자동 수령 후 노말 스테이로 자동 이동
...
- 게임화면의 중앙 상단에 stage, 적 HP, 보유 중인 재화량이 있다.
- 게임화면의 우측 상단에는 보스 타이머가, 하단에는 탭들이 있다.
- 설정에는 배경음 및 효과음, 게임종료, 계속하기가 있다.
- 배경음 및 효과음은 슬라이드로 조절한다.
- 게임종료 버튼은 연빨간색 배경에 흰색 글씨로 설정한다.
- 게임종료 버튼을 누를 경우, 게임이 종료된다.
...
- 가방창의 중앙 중단는 얻은 재화량 및 아이템(노말 스테이지 클리어 보상)을 표기한다.
- 가방창의 중앙 하단에 확인 버튼이 있다.
- 하단탭을 누를 경우, 창이 생성되며 이미지가 변경된다.
- 모든 하단창의 우측 상단에 아래방향 버튼이 있으며, 누르면 하단탭 창이 닫힌다.
- 성장탭의 좌측에는 각 성장 이미지가 있으며, 중앙에는 좌측 이미지의 설명, 우측에는 성장 비용이 있다.
- 캐릭터탭은 캐릭터를 고용할 수 있다.
- 캐릭터탭의 배열은 미정이다.
- 아이탬탭은 중앙에 얻은 아이템들이 있으며 하단에 아이템의 세부 설명이 적혀있다.
- 아이탬탭의 좌측 하단은 아이템 이미지, 중앙에 아이탬의 설명, 우측에는 사용하기 버튼이 있다.
- 상점탭은 기본적으로 구매탭으로 설정되어 있다.
- 상점창의 상단에는 구매, 판매가 있다.
- 상점탭은 중앙에 얻은 아이템들이 있으며 하단에 아이템의 세부 설명이 적혀있다.
- 상정탭의 좌측 하단은 아이템 이미지, 중앙에 아이탬의 설명, 우측에는 사용하기 버튼이 있다.
- 보스탭 창은 좌측에는 보스 몬스터 이미지, 중앙에는 보스 몬스터의 설명, 우측에는 이동하기가 있다.
- 이동하기 버튼을 누를 경우, 보스 스테이지로 이동한다.
- 보스과의 전투 성공 및 전투 실패 화면을 추가를 한다.
...
- 노말 스테이지 몬스터를 처치 시 재화를 1% 확률로 아이템을 랜덤드랍한다.
...
- 보스 스테이지 이동 시, 2초 간 로딩화면(검은 화면)이 나온 후, 게임화면으로 전환한다.
...
|노란색|몬스터를 기절 상태로 만든다.|3초간|모든 공격이 치명타로 들어간다(치명타 확률이 0%도 가능), 비행 중인 몬스터는 추락한다.|표시|
|초록색|||||
...
- 물약1은 초반에 효율이 좋고, 물약2는 후반에 효율이 좋도록 설정한다. <br>
...
- 안드로이드 빌드한다.(100%)
- 각 창이 겹치지 않도록 한다.(100%) 
- 설정버튼을 누를 경우, 창이 생성된다.(100%)
- 설정에는 배경음 및 효과음 조절, 게임종료 버튼, 계속하기 버튼이 있다.(100%)
- 배경음 및 효과음을 슬라이드로 조절한다. + 사운드 테스트(50%)
...
- 하단탭을 누를 경우, 창이 생성되며 이미지가 변경된다.(20%)
- 창의 우측 상단에 아래방향 버튼이 있다.(100%)
- 아래방향 버튼을 누를 경우, 창이 꺼진다.(100%)

[작업내용]

- 버튼 누를 경우, 창이 나오게 설정(100%)
- 사운드 조절(0%)
- 하단탭 클릭 시 이미지 변경(0%)
...
- 배경음 및 효과음을 슬라이드로 조절한다. + 사운드 테스트(100%)
...
- 1 스테이지 노말 몬스터, 보스 몬스터의 stand 상태를 구현한다.(20%) -> 노말 스테이지 몬스터는 기본적인 틀만 잡은 것임.
...
- 노말 스테이지 몬스터에서 보스 스테이지 몬스터로 변환한다.(0%) -> 보스 스테이지 몬스터를 미구현하여 못함.
...
- 철 골렘(보스 스테이지1 몬스터)의 방어 상태를 구현한다.(0%)
- 철 골렘의 방어 상태는 120초마다 사용한다.(0%)
...
- 성장탭에서 공격력 증가, 공격주기 감소, 치명타확률 증가, 치명타피해량 증가 버튼이 있다.
- 치명타확률은 1 ~ 100 사이의 숫자를 랜덤으로 뽑은 수가 치명타확률보다 크거나 같다면 캐릭터가 치명타 공격을 가한다.
- 치명타피해량은 노란색 텍스트로 피해량을 표기한다.
- 아이템탭에서 보유 중인 아이템이 있다.
- 물약1은 30분간 버프를 부여한다.
- 물약1의 종류는 공증, 공주감, 치증이 있다.
- 물약2는 5분간 버프를 부여한 후, 5분간 디버프를 부여한다.
- 물약2의 종류는 공증공주증, 치증치확0이 있다.
- 다른 종류의 물약(물약1 + 물약2)은 중첩이 가능하지만, 같은 종류의 물약(물약1 + 물약1)은 중첩이 불가능하다.
- 노말 스테이지 몬스터를 처치 시, 매우 낮은 확률(1%)로 아이템(물약1)을 드랍한다.
- 보스 스테이지 몬스터를 처치 시, 낮은 확률(10%)로 아이템(물약1, 물약2)을 드랍한다.
- 높은 노말 스테이지의 몬스터를 처치 시, 높아진 확률(10stage 당 0.1% 증가)로 아이템(물약1)을 드랍한다.
- 캐릭터의 버프창은 화면 중단 기준으로 좌측이고, 우측은 디버프창이다.
- 물약을 사용하면 버프창에 생성되며, 물약2의 디버프가 발동되면 디버프창에 생성된다.
- 상점탭 상단에 구매 및 판매 선택 버튼이 있다.
- 상점탭에 들어가면 기본적으로 구매 상태로 선택되어있다.
...
- 보스 스테이지 1 몬스터의 방어 상태 애니메이션 제작

### 아이템 시스템 구현(5주차 미구현)

[요구사항]

- 물약2는 5분간 버프를 부여한 후, 5분간 디버프를 부여한다.(20%)
- 물약2의 종류는 공증공주증, 치증치확0이 있다.(20%)
