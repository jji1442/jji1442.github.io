...
- imgAtkStop(검밴): 공격 불가능
- imgPtnStop(별): 패턴 파훼 불가능 -> 디버프 정상 표기 되는지 물약2로 테스트
-imgAtkStop - 검밴 - 공격 불가능 30 5
-imgPtnStop - 별 - 패턴 파훼 불가능 2
-게임 종료 시 디버프 저장을 안하게 해야함. v
-디버프 공격 중지 추가.v
-방향키 패턴 사라지게 추가. v
-스턴 애니메이션 추가,v
-방어막 애니메이션 제작 및 추가v
-환생 시스템 및 캐릭터 정보 시스템 이미지 제작한다.v
-보스 스테이지 설명에 회피도 추가.  v
...
- 공격력: 원거리 공격 가능.
- 공격속도: 4타 마다 2번 공격.(100%)
- 치명타확률: 추가 공격에 치명타 공격 가능.(100%)
- 치명타피해량: 미정
- 적중도는 레벨과 관계없이 적용.
- 적중도: 회피도의 120% 이상일 경우, 최종피해량 20% 증가(초과분에 비례하여 최대 50%)(100%)
...
#1 보상 훈장 추가 complate
1. 보스 보상 추가
결과창에 보상 출력v + 훈장 개수 증가 및 저장v
체력 %에 따른 보상 차이v

#2 아이템 창에 훈장이 보이게 설정. complate
1. iteminfo에 훈장 추가v
2. 설명은 사용 시 캐릭터 능력치 1렙업 가능v

#3 훈장 시스템 만들기
1. 사용 시, 성장 1단계 가능 v
'재화비용' -> 'Level UP!' 으로 변경. 재화는 소모되지 않게 설정.
-> 현재 레벨에 훈장을 사용하는 것이므로 레벨업 재화비용은 상승한다.
-> ex) 성장 비용이 300 + 3%일 때,
         1레벨에 훈장 사용하여 강화하면 2레벨에 300이 아니라 303이다.

아이템 사용 함수에서 isUsedMedal = true, 성장탭 성장비용 text 변경 v+ 저장 -> 오류 남
성장탭에서 isUsedMedal에 따라 비용 사용할지 말지 정함.v
isUsedMedal 저장v

레벨업 후, 비용 변경v

#4. 훈장이 사용되어 있을 경우 v
화면에 훈장이 이미 사용된 상태입니다.
출력한다.
isUsedMedal <- check
