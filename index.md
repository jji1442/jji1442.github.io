...
11. [프로토타입 개발 작업일정(6주 개발)](#프로토타입-개발-작업일정6주-개발)
    ...
    - 5주차
    - 6주차
12. [알파 개발 요구사항(10주 개발)](#알파-개발-요구사항10주-개발)
13. [알파 개발 작업일정(10주 개발)](#알파-개발-작업일정10주-개발)
14. [변경 및 폐기된 아이디어들](#변경-및-폐기된-아이디어들)
15. [고민 중인 사항](#고민-중인-사항)
...
- 캐릭터가 일정한 능력치를 달성하면, 몬스터와의 상성이 변화하는 추가 효과를 부여하여 게임의 전략적인 측면을 강화한다. 이러한 시스템을 통해 플레이어는 캐릭터의 성장에 따라 게임 플레이에 더욱 차별화된 전략을 구사할 수 있게 된다.

### 컨셉2: 낮은 진입장벽

- 이 게임은 시작하는 데 필요한 조작이나 지식이 적기 때문에, 신규 플레이어도 쉽게 게임에 진입할 수 있습니다. 또한, 게임 내에서는 무한한 성장이 가능하기 때문에, 플레이어들은 계속해서 새로운 도전과 성취감을 느낄 수 있습니다. 이러한 성장 요소들은 게임의 재미와 흥미를 유지시키는데 큰 역할을 합니다.
...
- 성장: 캐릭터 능력치 증가한다.
- 밸런스: 능력치별 증가 수치 조정한다.
- 선택: 성장탭에서 어떤 능력치를 성장할 지 고민한다.
- 시간제한: 보스 스테이지에서 화면 우측 상단 타이머를 활성화한다.
- 보스패턴: 보스 스테이지의 몬스터 패턴 클리어 성공/실패 시 리턴을 준다.
- 아이템: 아이템탭에서 아이템 사용한다.
...
- 보스 난이도가 높을 경우, 여러 번 도전하여 페이즈 별 보상인 훈장으로 보스 난이도를 낮출 수 있다.
...
- 중학생 ~ 고등학생 때에 ‘TapTitans’라는 게임을 자주 플레이했었다.
...
- 캐릭터 성장 변화
	...
	- 이러한 추가적인 효과는 게임의 전략적인 측면을 강화시키는 역할한다.
...
- 보스 타이머가 분당 10초 미만일 때, 초단위 앞에 0 붙힌다.(100%)
	- 수정 전: 9 : 7
	- 수정 후: 9 : 07
- 보스 스테이지에서 종료 시 노말 스테이지 초기화한다.(100%)
...
- 패턴 중 플레이어의 캐릭터들은 공격을 멈춘다(imgAtkStop).(100%)
- 플레이어의 디버프에 기절상태(imgAtkStop)를 표기한다.(100%)
- 입력할 방향키 맞을 경우, 상단에 입력할 방향키 이미지가 사라진다.(100%)
- 입력할 방향키 틀린 경우, 상단에 입력할 방향키가 초기화되며, 플레이어의 캐릭터가 2초간 기절상태(imgPtnStop)가 된다.(100%)
- 플레이어의 디버프에 기절상태(imgPtnStop)를 표기한다.(100%)
- 캐릭터 위에 기절 상태 애니메이션을 표기한다.(100%)
- 시간제한 내에 성공 시, 10초간 피격 피해량 10% 증가한다.(100%)
- 몬스터 디버프 창에 피격 피해량 증가 표기한다.(100%)
- 시간제한 초과 시, 플레이어의 캐릭터가 5초간 기절상태(imgAtkStop)가 된다.(100%)
- 플레이어의 디버프에 기절상태(imgAtkStop)를 표기한다.(100%)
- 패턴 종료 시, 하단 부분의 방향키는 사라진다.(100%)
- 보스에서 게임 종료 시, 패턴 관련 디버프는 사라진다.(100%)

- 아이템 창에 훈장을 추가한다.(100%)
- 훈장 아이템의 설명에 '사용 시 캐릭터 능력치 1레벨 업 가능'을 표기한다.(100%)
- 훈장 아이템 사용 시. '각 능력치의 성장비용'을 'Level UP!'으로 변경한다.(100%)
- 현재 레벨에 훈장을 사용하는 것이므로 레벨업 후 성장비용은 상승한다.(100%)
	- ex) 성장비용이 300 + 3% 일 때, 0레벨에 훈장을 사용하여 강화하면, 1레벨에 성장비용이 300이 아니라 303이다.
- 훈장 아이템을 이미 사용했을 경우, 아이템 중복 알림을 출력한다.(100%)
- 보스 체력 %에 따른 보상 차이를 구현한다.(100%)
- 전투 결과창의 보상에 훈장을 출력한다.(100%)
- 전사 캐릭터 능력치에 따른 추가효과 구현한다.(100%)

[작업내용]

- 시스템 이미지 제작 및 개선(100%)
- 버프창 이미지 제작(100%)
- 1 스테이지 보스 몬스터 방향키 패턴 구현(100%)
- 아이템(훈장) 구현(100%)
- 보스 스테이지 보상(훈장) 추가(100%)
...
- 노말 2스테이지 몬스터를 제작한다.
- 노말 2스테이지 몬스터의 패시브(2초당 10 회복)를 제작한다.

- 영입탭에서 궁수 영입이 가능하게 제작한다.
- 궁수 스탯 시스템을 구현한다.
- 궁수 공격 시스템을 구현한다.
- 성장탭에서 궁수 강화 시스템을 구현한다.
- 궁수 성장 저장시스템 구현한다.
- 능력치 패널에서 전사/궁수 선택하는 버튼을 제작한다.
- 성장탭에서 전사/궁수 선택하는 버튼을 제작한다.
- 궁수 추가 능력 구현한다.
...
- 자동 1인 지정 공격 시스템을 제작한다.
- 대상 지정하는 시스템을 제작한다.
- 대상 지정 이펙트를 제작한다.

- 2스테이지 보스탭 설명 변경한다.
- 2스테이지 보스 스테이지로 이동을 구현한다.
- 2스테이지 보스 스탯 제작한다.
- 보스 패턴1을 구현한다.
- 보스 패턴2를 구현한다.
- 몬스터 2마리 체력 시스템을 구현한다.
...
- 1. 1stage 보스 변경사항
1) 보호막과 피하기 패턴 2가지를 랜덤하게 사용하게 설정.
2) 보호막 패턴 시, 가운데에 터치 버튼 및 타이머를 생성 후, 터치할 때 마다 일정한 피해를 입히게 설정.
3) 보호막이 다 사라졌을 경우, 버튼은 사라짐.
4) -> 기대결과. 플레이어의 개입량 증가.
5) 피하기 패턴은 3초의 시간을 주며, 보스가 애니메이션에 따라 플레이어가 피한다.
6) 이를 보스 체력에 따라 6/12회를 회피하면 된다.
7) -> 기대결과. 보스의 역동적임을 보여주기 위함.
8) 이 보호막 패턴과 피하기 패턴 두 가지를 3번 실패 시 보스 스테이지 클리어 실패를 표기한다.

- 2. 시스템 변경사항.
1) 보스 스테이지 패배 시 패배 이유를 적어 줄 것.
2) 캐릭터 능력치 창에 개별 버프창 표기할 것.
3) -> 이유. 캐릭터 능력치 창에 있는 수치는 증감 수치 전부를 표기했지만. 어떤 디버프인지 알 수가 없음.
4) 추가적으로 버프/디버프를 클릭 시 설명 표기.
5) ex) 00물약으로 인해 공격속도가 감소합니다! 공격속도 0% 감소. 이러한 것들.
6) 환생 시스템의 변경 고려 중.

- 3. 기본 변경 사항들
1) 몬스터의 피격 모션 변경(피격 이펙트(빨간색 점등 방식) -> 애니메이션)
2) 몬스터에게 피격 피해량 텍스트 변경(텍스트가 이동하게 변경 및 2타 공격 구현)
