...
- 전투퇴장 버튼을 누를 경우, 로딩화면 2초간 출력 후, 노말 스테이지로 이동한다.

- 보스과의 전투승리/전투패배 시 각 창이 출력된다.
- 전투승리 창의 중앙에 얻은 금화량, 얻은 아이템, 돌아가기가 있다.
- 전투패배 창의 하단에 다시하기, 돌아가기가 있다.
- 다시하기 버튼을 누를 경우, 로딩화면 2초간 출력 후, 보스 스테이지를 리셋한다.
- 돌아가기 버튼을 누를 경우, 로딩화면 2초간 출력 후, 노말 스테이지로 이동한다.
...
- 보스탭에서 보스 선택, 보스 스테이지로 이동한다.
- 보스 스테이지 이동할 경우, 2초 간 로딩화면이 나온 후, 게임화면으로 전환한다.
- 보스 스테이지 이동할 경우, 타이머가 활성화된다.
- 보스 스테이지에서는 상점탭과 보스탭 사용할 수 없다.
- 보스 스테이지를 클리어 성공/실패 시 캐릭터에 존재하는 모든 상태가 풀린다.
- 보스 스테이지 몬스터를 처치 시 아이템과 재화를 얻는다.
- 보스 스테이지에서 드랍하는 아이템은 난이도에 따라 보상이 달라진다.
- 보스 스테이지에서 전투퇴장이 있다.
- 전투퇴장 버튼을 누를 경우, 노말 스테이지로 이동하며 노말 스테이지 1단계로 초기화된다.
- 보스 체력이 0 이하가 되면, 전투승리 패널이 생성된다.
- 전투승리 패널에는 중앙에 전투승리 텍스트, 얻은 재화량, 얻은 아이템 종류, 돌아가기가 있다.
- 아이템은 각 50% 확률로 아이템 종류로 얻을 수 있다.
- 이 보상은 자동으로 수령 후, 노말 스테이지로 이동 및 노말 스테이지 1단계로 초기화된다.
- 보스 타이머가 0초 이하가 되면, 노말 스테이지로 이동 및 노말 스테이지 1단계로 초기화된다.
