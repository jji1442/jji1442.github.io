...
- 게임화면에는 좌측 상단에 순서대로 설정, 가이드북, 스테이지 보상을, 중앙 상단에 순서대로 stage, 적 HP, 보유 중인 재화량을, 우측 상단에는 보스 타이머를 두며, 하단에 탭들을 둔다.
...
- 가이드북
	- 1. 튜토리얼 재설명
	- 2. 몬스터에 대한 정보 및 이스터 에그를 제외하고 모든 것에 대한 설명

- 일반피해량은 흰색 텍스트, 치명타피해량은 노란색 텍스트를 출력한다.
- 아이템탭 보유 중인 아이템을 보여준다.
- 경감은 입는 피래량을 감소시킨다.
