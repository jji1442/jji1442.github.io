...
# [요구사항]
시작화면, 게임화면이 있다.
시작화면에는, 시작하기, 설정, 종료 버튼이 있다.
설정에는 밝기, 소리를 조절하며, 뒤로가기, 게임종료 버튼이 있다.
시작화면에 시작하기를 누르면 게임화면으로 변경된다.
노말 스테이지의 적을 처치 시 낮은 확률로 아이템을 드랍한다.
보스 스테이지를 클리어 실패 시 캐릭터에 존재하는 모든 상태가 풀린다.
보스 스테이지에서 노말 스테이지로 이동(전투 퇴장)하는 버튼을 추가한다.
보스 스테이지를 어떠한 방법으로든 벗어날 경우, 노말 스테이지는 1단계로 초기화된다.
보스 스테이지에서는 상점탭과 보스 스테이지 이동탭 이용 불가능하다.
