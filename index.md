...
|캐릭터 종류|캐틱터 강화|강화주기(최대 20초)|정화주기(최대 40초)|
|:----:|:----:|:----:|:----:|
|지원형_마법사|공격력 1 증가, 치명타 피해량 1% 증가|0.5초 감소|1초 감소|
...
|남색|몬스터에게 피해를 입힌다.|해당사항 없음|모든 캐릭터의 공격력에 비례한 피해를 입힌다.|미표시|
|보라색|몬스터를 부식시킨다.|해당사항 없음|부식은 경감 1단계를 내린다.|미표시|
...
### 전사 캐릭터, 1 스테이지 노말 몬스터 및 보스 몬스터의 stand 상태 구현
...
### 전사 캐릭터 공격 상태, 1 스테이지 보스 몬스터 패턴 구현

- 전사 캐릭터 공격 상태를 구현한다.
- 보스 몬스터 피격 모션 구현
- 몬스터 기절 모션 구현
- 1 스테이지 보스 몬스터인 철 골렘의 방어 패턴을 구현한다.
- 몬스터의 체력을 구현한다.
- 몬스터에게 입히는 피해량을 표기를 구현한다.
- 노말 스테이지의 몬스터를 처치 시, 노말 스테이지 증가, 가방에 보상 적립, 몬스터의 체력을 증가하게 구현한다.
- 보스 스테이지의 몬스터를 처치 시, 보상을 주어진다.
...
- 가방 구현 노말 스테이지 보상 구현한다.
- 캐릭터의 버프창은 화면 중단 기준으로 좌측은 버프고, 우측은 디버프다.
- 몬스터의 버프창은 체력 기준으로 좌측은 버프고, 우측은 디버프다.
- 성장탭에 캐릭터의 성장 종류, 성장 수치 및 버튼 구현한다.
- 아이템탭 보유 중인 아이템을 보여준다.
- 아이템탭: 아이템 사용효과 구현한다.
- 상점탭: 아이템 구매 및 판매 구현한다.
- 폭탄은 전사 캐릭터 기준으로 몬스터를 대상으로 던진다.
...
### 전사 캐릭터, 1 스테이지 노말 몬스터 및 보스 몬스터의 stand 상태 구현(3주차)
