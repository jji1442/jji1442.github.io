...
> ### 대표이미지 기반 : 좌측 상단에 순서대로 설정, 스테이지 보상을, 중앙 상단에 순서대로 stage, 적 HP, 보유 중인 재화량을, 우측 상단에는 보스 타이머를 두며, 하단에 탭들(캐릭터탭, 성장탭, 아이템탭, 상점탭, 보스탭)을 둘 예정이다.

> ### 컨셉 기반 : 노말 스테이지에서 적을 쓰러트리고 재화 및 아이템을 얻어 얻은 재화로 성장하여 정해진 시간 내에 보스 스테이지를 클리어한다.
