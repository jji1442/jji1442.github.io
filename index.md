...
|전사|400|5초|0%|400%|
|궁수|600|7초|0%|600%|
|공격형_마법사|1000|10초|0%|800%|

|캐릭터 종류|강화수치|강화주기|지속시간|정화주기|
|:----:|:----:|:----:|:----:|:----:|
|지원형_마법사|공격력 20 증가, 치명타피해량 40% 증가|60초|10초|120초|
...
|궁수|15 증가|0.1초 감소(최대 5초 감소)|1% 증가(최대 100%)|30% 증가|
|공격형_마법사|20 증가|0.1초 감소(최대 5초 감소)|1% 증가(최대 100%)|40% 증가|

|캐릭터 종류|강화수치|강화주기(최대 20초)|정화주기(최대 40초)|
|:----:|:----:|:----:|:----:|
|지원형_마법사|공격력 2 증가, 치명타피해량 4% 증가|0.5초 감소(최대 10초 감소)|1초 감소(최대 20초 감소)|
...
- 보스 스테이지 구현
- 보스 패턴
- 보스는 30초마다 랜덤으로 패턴을 실행한다.
- 30초의 기준은 패턴이 끝난 후 기준이다.
- 패턴 나오기 1초 전에 대사가 나온다. -> 확정 아님
- 1. 방향키 패턴
- 하단 부분에 방향키가 생기며, 상단 중앙에 나온 방향키 패턴을 눌러 파훼한다.
- 시간 내 성공 시, 전투가 계속 이어질 수 있으며,
- 시간 내 실패 시, 기절 상태 5초간 부여한다.
- 추가적으로 패턴 중에 캐릭터의 공격은 멈춘다.
- 2. 보호막 패턴
- 패턴 시, 보호막을 부여한다.
- 보호막: 모든 피해를 흡수하는 방어막을 생성한다.
- 3. 경감 패턴
- 패턴 시, 보스에게 경감 1중첩을 부여한다.
- 보스 버프창에 경감 중첩을 표기한다.
- 경감: 중첩당 피해량 감소 5%, 최대 6중첩이다.

[작업내용]

- 몬스터 피격 모션 개선(100%)
- 물약 위치 재배치
- 환생 및 캐릭터 능력치 확인 이미지 추가
- 1 스테이지 보스 몬스터 구현
