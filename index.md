...
|:----:|:----:|:----:|
|기본|stand|아무 행동하지 않는 모습|
|배경 점액질 생성||일정 시간마다 필드의 점액질이 커지는 모습|
|필드 점액질 생성||킹 슬라임이 일정 체력 이하로 떨어지면 필드에 점액질을 생성하는 모습|
|점프!|jump!|킹 슬라임이 점프를 준비한 후 일정 체력 이상 피격 실패 시 점프하는 모습|
|분열|divideOneself|킹 슬라임의 체력이 0이 되어 슬라임 2체로 분열하는 모습|
|슬라임 패턴||각 슬라임에게는 능력이 부여되어 있다.|
