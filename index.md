...
- 전사 캐릭터 공격 상태를 구현한다.
- 1 스테이지 노말 몬스터 및 보스 몬스터 피격 상태를 구현한다.
- 1 스테이지 보스 몬스터인 철 골렘의 방어 상태를 구현한다.
- 몬스터의 체력 및 몬스터에게 입히는 피해량을 구현한다.
- 노말 스테이지의 몬스터를 처치 시, 노말 스테이지 증가, 가방에 보상 적립, 몬스터의 체력을 증가를 구현한다.
...
- 성장탭에는 캐릭터의 성장 종류, 성장 수치 및 버튼 구현한다.
- 치명타피해량을 구현한다.
- 아이템탭에 보유 중인 아이템을 구현한다.
- 아이템(물약1, 물약2, 폭탄) 사용효과 구현한다.
- 폭탄은 그 중 보라색만 구현할 예정이다.
- 폭탄은 보스전에서만 사용 가능하게 구현한다.
- 캐릭터의 버프창은 화면 중단 기준으로 좌측은 버프고, 우측은 디버프다.
- 몬스터의 버프창은 체력 기준으로 좌측은 버프고, 우측은 디버프다.
- 몬스터 기절 모션 구현한다.
- 상점탭에는 아이템 구매 및 판매 구현한다.
