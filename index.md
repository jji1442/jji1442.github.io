...
|캐릭터 종류|공격력|공격주기|치명타확률|치명타피해량|적중도|
|:----:|:----:|:----:|:----:|:----:|:----:|
|전사|300 + 14.3%|500 + 12.5%|500 + 12.5%|300 + 14.3%|300 + 14.3%|
|궁수|700 + 14.3%|1,200 + 12.5%|1,200 + 12.5%|700 + 14.3%|700 + 14.3%|
|공격형_마법사|1,500 + 14.3%|2,000 + 12.5%|2,000 + 12.5%|1,500 + 14.3%|1,500 + 14.3%|

|캐릭터 종류|강화수치|강화주기|정화주기|
|:----:|:----:|:----:|:----:|
|지원형_마법사|50,000 + 20%|50,000 + 20%|50,000 + 20%|
...
- 캐릭터 능력치 창에서 검은색은 기본 능력치 + 성장 능력치, 초록색은 버프 수치, 빨간색은 디버프 수치다.(100%)
- 캐릭터 성장탭 창에서 전체 캐릭터 능력치가 아닌 성장 능력치 수치로 나타나게 변경한다.(100%)
- ex) 수정 전: 캐릭터 기본 스탯 + 성장 수치 * 레벨 -> 캐릭터 기본 스탯 + 성장 수치 * (레벨 + 1)
- ㅁㅁ- 수정 후: 성장 수치 * 레벨 -> 성장 수치 * (레벨 + 1)
- 성장탭은 스크롤 바를 사용하는 방식으로 변경한다.
...
- 시간제한 내에 보호막을 파괴 시, 10초간 피격 피해량 10% 증가한다.
- 몬스터 디버프 창에 피격 피해량 증가 표기한다.
- 시간제한 내에 보호막을 파괴하지 못할 시, 보호막은 유지된 상태로 경감이 1스택이 중첩된다.
- 몬스터 버프창에 경감상태를 표기한다.

- 2페, 3페(50%, 20%)
- 방향키 패턴의 애니메이션을 추가한다.
- 중앙에 30초의 시간제한이 표기한다.
- 하단 부분에 방향키가 생기며, 상단에 입력할 방향키를 랜덤으로 생성한다.
- 패턴 중 플레이어의 캐릭터들은 공격을 멈춘다.
- 입력할 방향키 맞을 경우, 방향키 이미지가 사라진다.
- 입력할 방향키 틀린 경우, 입력할 방향키가 초기화되며, 플레이어의 캐릭터가 2초간 기절상태가 된다.
- 시간제한 내에 성공 시, 10초간 피격 피해량 10% 증가한다.
- 몬스터 디버프 창에 피격 피해량 증가 표기한다.
- 시간제한 초과 시, 플레이어의 캐릭터가 5초간 기절상태가 된다.
- 플레이어의 디버프에 기절상태를 표기한다.
- 패턴 종료 시, 하단 부분의 방향키는 사라진다.
