...
- 설정에는 밝기 및 소리 조절, 확인 버튼이 있다.
- 밝기 및 소리의 조절 단계는 5단계다.
- 확인 버튼을 누를 경우, 설정 창이 닫힌다.
- 가방을 누를 경우, 창이 생성된다.
- 가방 창에는 얻은 재화량 및 아이템을 표기한다. 
- 창의 중앙하단에 확인 버튼이 존재한다.
- 하단탭을 누를 경우, 창이 생성된다.
- 창의 우측 상단에 아래방향 버튼이 존재한다.
...
- 보스 스테이지 이동 시, 2초 간 로딩화면(검은 화면, 추후 추가 예정)이 나온 후, 게임화면으로 전환한다.
...
- 노말 스테이지의 몬스터를 처치 시, 노말 스테이지가 증가한다.
- 노말 스테이지의 몬스터 및 보스 스테이지 몬스터의 체력을 구현한다.
- 보스 몬스터에게 입히는 피해량을 표기를 구현한다.
