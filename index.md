...
- 몬스터의 체력이 0 이하가 되면 보상이 주어지며, 이 보상은 가방에 누적 후, 스테이지 1단계 증가 및 더 높은 체력을 가진 몬스터를 생성
- 가방을 클릭 시, 노말 스테이지에 대한 보상을 받게 되고 노말 스테이지 1단계로 초기화

### 보스 스테이지

- 보스 체력이 0 이하가 되면, 보상이 주어지며, 이 보상은 자동으로 수령 후, 노말 스테이지로 이동 및 노말 스테이지 1단계로 초기화
- 보스 타이머가 0초 이하가 되면, 노말 스테이지로 이동 및 노말 스테이지 1단계로 초기화
...
- 물약1 또는 물약2를 선택하여 사용하기 버튼을 누를 경우, 플레이어의 버프창에 사용한 물약 이미지가 생성되며 남은 시간(분/초)이 출력된다.
...
- 보스 스테이지로 이동할 경우, 좌측 상단에 전투퇴장이, 우측 상단에 타이머가 활성화되며, 가방, 상점탭, 보스탭이 비활성화된다.
...
|:----:|:----:|:----:|:----:|:----:|
|지원형_마법사|공격력 15 증가, 치명타피해량 10% 증가|60초|10초|120초|
...
|캐릭터 종류|공격력|공격주기|치명타확률|치명타피해량|
|:----:|:----:|:----:|:----:|:----:|
|전사|10 증가|0.1초 감소(최대 3초 감소)|1% 증가(최대 100%)|20% 증가|
|궁수|6 증가|0.5% 감소|1% 증가|10% 증가|
|공격형_마법사|10 증가|0.3% 감소|1% 증가|10% 증가|

|캐릭터 종류|강화수치|강화주기(최대 20초)|정화주기(최대 40초)|
|:----:|:----:|:----:|:----:|
|지원형_마법사|공격력 1 증가, 치명타피해량 1% 증가|0.5초 감소|1초 감소|
...
|공격형_마법사|4000 + 20%|4000 + 20%|2000 + 20%|4000 + 20%|
...
- ~~배경화면를 구현한다.~~
- ~~설정, 가방, 스테이지, 적 체력, 재화, 타이머를 구현한다.~~
- ~~하단탭(성장탭, 캐릭터탭, 아이템탭, 상점탭, 보스탭)를 구현한다.~~
...
- ~~안드로이드 빌드한다.~~
- ~~각 창이 겹치지 않도록 한다.~~
- ~~설정 버튼을 누를 경우, 창이 생성된다.~~
- ~~설정에는 배경음 및 효과음 조절, 게임종료 버튼, 계속하기 버튼이 있다.~~
- 배경음 및 효과음을 슬라이드로 조절한다. + 사운드 테스트
- ~~게임종료 버튼을 누를 경우, 게임이 종료된다.~~
- ~~게임종료 버튼은 연빨간색 배경에 흰색 글씨로 강조한다.~~
- ~~계속하기 버튼을 누를 경우, 설정 창이 닫힌다.~~
- ~~가방 버튼을 누를 경우, 창이 생성된다.~~
- ~~가방 창에는 얻은 재화량 및 아이템을 표기한다.~~
- ~~창의 중앙하단에 확인 버튼이 있다.~~
- 하단탭을 누를 경우, 창이 생성되며 이미지가 변경된다.
- ~~창의 우측 상단에 아래방향 버튼이 있다.~~
- ~~아래방향 버튼을 누를 경우, 창이 꺼진다.~~
...
- 1 스테이지 노말 몬스터, 보스 몬스터의 stand 상태를 구현한다.
- 보스탭에서 보스 선택 시, 보스 스테이지로 이동한다.
- 보스 스테이지 이동 시, 2초간 로딩화면(검은 화면)이 나온 후, 게임화면으로 전환한다.
- 노말 스테이지 몬스터에서 보스 스테이지 몬스터로 변환한다.
- 스테이지 표기(Stage 01 -> Boss Stage 01)를 변경한다.
- 보스 타이머 및 텍스트를 활성화한다.
- 상점탭과 보스탭을 사용할 수 없다.
- 노말 스테이지로 이동 버튼을 화면 중앙에 활성화한다.
...
- 1 스테이지 노말 몬스터 및 보스 몬스터의 피격 상태를 구현한다.
- 1 스테이지 보스 몬스터의 방어 상태를 구현한다.
- 전사 캐릭터의 5초마다 공격한다.
- 보스 몬스터인 철 골렘의 방어 상태는 120초마다 사용한다.
- 몬스터의 체력 및 몬스터에게 입히는 피해량을 구현한다.
- 노말 스테이지의 몬스터를 처치 시, 노말 스테이지 증가, 가방에 보상 적립, 몬스터의 체력을 증가한다.
...
- 성장탭에서 공격력 증가, 공격주기 감소, 치명타확률 증가, 치명타피해량 증가 버튼을 선택하여 성장한다.
- 치명타 피해량을 구현한다.
- 아이템탭에서 보유 중인 아이템을 보여준다.
- 아이템(물약1, 물약2) 사용효과를 구현한다.
- 캐릭터의 버프창은 화면 중단 기준으로 좌측이고, 우측은 디버프창이다.
- 몬스터의 버프창은 체력 기준으로 좌측이고, 우측은 디버프창이다.
- 물약을 사용하면 캐릭터의 버프창에 생성된다.
- 보스 방어 상태일 시, 버프창에 생성된다.
- 폭탄을 사용하면 몬스터의 디버프창에 생성된다.
- 상점탭에는 물약1 구매 및 물약1, 물약2 판매가 가능하다.
...
- 설정 버튼을 누를 경우, 창이 생성된다.(100%)
- 설정에는 배경음 및 효과음 조절, 게임종료 버튼, 계속하기 버튼이 있다.(100%)
- 배경음 및 효과음을 슬라이드로 조절한다.(50%)
- 게임종료 버튼을 누를 경우, 게임이 종료가 된다.(100%)
- 게임종료 버튼은 연빨간색 배경에 흰색 글씨로 강조한다.(100%)
- 계속하기 버튼을 누를 경우, 설정 창이 닫힌다.(100%)
- 가방 버튼을 누를 경우, 창이 생성된다.(100%)
- 가방 창에는 얻은 재화량 및 아이템을 표기한다.(100%)
- 창의 중앙하단에 확인 버튼이 있다.(100%)
- 하단탭을 누를 경우, 창이 생성되며 이미지가 변경된다.(0%)
- 창의 우측 상단에 아래방향 버튼이 있다.(100%)
- 아래방향 버튼을 누를 경우, 창이 꺼진다.(100%)

[작업내용]

- 하단탭 버튼을 누를 경우, 창이 나오게 설정(100%) -> 모든 하단탭 설정
- 사운드 조절(70%)
- 하단탭 클릭 시 이미지 변경(0%)
...
- 배경음 및 효과음을 슬라이드로 조절한다.(100%)
- 하단탭을 누를 경우, 창이 생성되며 이미지가 변경된다.(100%)
...
- 1 스테이지 노말 몬스터, 보스 몬스터의 stand 상태를 구현한다.(0%)
- 보스탭에서 좌측은 몬스터 이미지, 중앙에는 보스 스테이지 번호, 우측에는 이동하기 버튼이 있다.(100%)
- 보스탭에서 보스를 선택하고 이동하기 버튼을 누를 경우, 보스 스테이지로 이동한다.(100%)
- 보스 스테이지 이동 시, 2초간 로딩화면(검은 화면)이 나온 후, 게임화면으로 전환한다.(100%)
- 노말 스테이지 몬스터에서 보스 스테이지 몬스터로 변환한다.(0%)
- 스테이지 표기(Stage 01 -> Boss Stage 01)를 변경한다.(100%)
- 보스 타이머 및 텍스트를 활성화한다.(100%)
- 상점탭과 보스탭을 비활성화한다.(100%)
- 노말 스테이지로 이동 버튼을 화면 중앙에 활성화한다.(100%)
- 노말 스테이지로 이동 버튼을 누를 경우, 노말 스테이지로 이동한다.(100%)
...
- 1 스테이지 노말 몬스터, 보스 몬스터의 stand 상태를 구현한다.(100%)
- 노말 스테이지 몬스터에서 보스 스테이지 몬스터로 변환한다.(100%)
...
- 1 스테이지 보스 몬스터의 방어 상태를 구현한다.(0%)
- 전사 캐릭터의 5초마다 공격한다.(100%)
- 보스 몬스터인 철 골렘의 방어 상태는 120초마다 사용한다.(0%)
- 몬스터의 체력 및 몬스터에게 입히는 피해량을 구현한다.(100%)
- 노말 스테이지의 몬스터를 처치 시, 노말 스테이지 증가, 가방에 보상 적립, 몬스터의 체력을 증가한다.(100%)
- 시작 화면 및 보스 스테이지 클리어 화면 출력한다.
- 노말 및 보스 스테이지 몬스처 처치 모션 제작한다.

[작업내용]

- 캐릭터 공격 구현 및 보스 패턴 구현
- 시작 화면 및 보스 스테이지 클리어 화면 구현
- 기본적인 게임 시스템 구현
...
- 철 골렘(보스 스테이지1 몬스터)의 방어 상태를 구현한다.(100%)
- 철 골렘의 방어 상태는 60초마다 사용한다.(100%)

[작업내용]

- 스테이지1 보스 몬스터 패턴 구현(100%)

### 아이템 시스템 구현(5주차)

[요구사항]

- 성장탭에서 공격력 증가, 공격주기 감소, 치명타확률 증가, 치명타피해량 증가 버튼이 있다.(100%)
- 성장의 상한선 설정/////////////
- 치명타확률은 1 ~ 100 사이의 숫자를 랜덤으로 뽑은 수가 치명타확률보다 크거나 같다면 캐릭터가 치명타 공격을 가한다.(100%)
- 치명타피해량은 노란색 텍스트로 피해량을 표기한다.(100%)
- 아이템탭에서 보유 중인 아이템이 있다.(100%)
- 물약1은 30분간 버프를 부여한다.(100%)
- 물약1의 종류는 공증, 공주감, 치증이 있다.(100%)
- 물약2는 5분간 버프를 부여한 후, 5분간 디버프를 부여한다.(0%)
- 물약2의 종류는 공증공주증, 치증치확0이 있다.(0%)
- 다른 종류의 물약(물약1 + 물약2)은 중첩이 가능하지만, 같은 종류의 물약(물약1 + 물약1)은 중첩이 불가능하다.(0%)
- 노말 스테이지 몬스터를 처치 시, 매우 낮은 확률(1%)로 아이템(물약1)을 드랍한다.(0%)
- 보스 스테이지 몬스터를 처치 시, 낮은 확률(10%)로 아이템(물약1, 물약2)을 드랍한다.(0%)
- 높은 노말 스테이지의 몬스터를 처치 시, 높아진 확률(10stage 당 0.1% 증가)로 아이템(물약1)을 드랍한다.(0%)
- 캐릭터의 버프창은 화면 중단 기준으로 좌측이고, 우측은 디버프창이다.(0%)
- 물약을 사용하면 버프창에 생성되며, 물약2의 디버프가 발동되면 디버프창에 생성된다.(0%)
- 상점탭 상단에 구매 및 판매 선택 버튼이 있다.(0%)
- 상점탭에 들어가면 기본적으로 구매 상태로 선택되어있다.(0%)
...
- 다른 종류의 물약(물약1 + 물약2)은 중첩이 가능하지만, 같은 종류의 물약(물약1 + 물약1)은 중첩이 불가능하다.(100%)
- 보스 스테이지 몬스터를 처치 시, 낮은 확률(10%)로 아이템(물약1, 물약2)을 드랍한다.(100%)
- 높은 노말 스테이지의 몬스터를 처치 시, 높아진 확률(10stage 당 0.1% 증가)로 아이템(물약1)을 드랍한다.(100%)
- 캐릭터의 버프창은 화면 중단 기준으로 좌측이고, 우측은 디버프창이다.(100%)
- 물약을 사용하면 버프창에 생성되며, 물약2의 디버프가 발동되면 디버프창에 생성된다.(100%)
- 상점탭 상단에 구매 및 판매 선택 버튼이 있다.(100%)
- 상점탭에 들어가면 기본적으로 구매 상태로 선택되어있다.(100%)
...
- 게임 시작 시, 시작화면을 출력한다.
- 시작화면에는 설정 및 게임 시작 버튼이 있다.
- 설정 버튼 누르면 설정창이 나온다.
- 게임시작 버튼을 눌렀을 때 게임화면으로 이동한다.
- 노말 스테이지 보스와 보스 스테이지의 사운드를 다르게 설정한다.(100%)
- 타격음 추가한다.(100%)
- 저장 시스템 구현한다.(100%)
