...
- 치명타피해량 공식
	- 치명타피해량 = (기본 치명타피해량 + 능력치로 증가된 치명타피해량) * 물약으로 증가된 치명타피해량(%)
...
- 1. 시작화면, 게임화면이 있다.
- 2. 시작화면에는, 시작하기, 설정, 종료 버튼이 있다.
- 3. 설정에는 밝기, 소리를 조절하며, 뒤로가기, 게임종료 버튼이 있다.
- 4. 시작화면에 시작하기를 누르면 게임화면으로 변경된다.
- 5. 노말 스테이지의 몬스터 처치 시 매우 낮은 확률로 아이템을 드랍한다.
- 6. 보스 스테이지를 클리어 성공/실패 시 캐릭터에 존재하는 모든 상태가 풀린다.
- 7. 보스 스테이지에서 노말 스테이지로 이동(전투 퇴장)하는 버튼을 추가한다.
- 8. 보스 스테이지를 어떠한 방법으로든 벗어날 경우, 노말 스테이지로 이동하며 1단계로 초기화된다.
- 9. 보스 스테이지에서는 상점탭과 보스 스테이지 이동탭 사용할 수 없다.
- 10. 보스 스테이지에서 보스가 생성된 후 보스 타이머를 작동한다.
- 11. 하단 탭을 클릭 시 탭에 대한 창이 나온다.
- 12. 성장탭은 캐릭터를 선택하여 성장시킬 수 있다.
- 13. 캐릭터탭은 캐릭터를 고용할 수 있다.
- 14. 아이템탭에서는 자신이 획득한 아이템을 확인 및 사용할 수 있다.
- 15. 상점탭에는 폭탄 및 일부 포션을 구매가 가능하다.
- 16. 보스탭은 원하는 보스를 선택하여 전투를 할 수 있다.
