...
- 뒤로가기 버튼을 누를 경우, 각 화면에서 설정 눌렀던 위치로 돌아간다.(시작화면에서 설정 누르고 뒤로가기 눌렀을 경우, 시작화면으로 돌아감.)
- 시작 후 로딩화면을 출력한다.
- 로딩화면 출력 후 게임화면이 출력한다.
- 시작화면에 시작하기를 누르면 게임화면으로 변경된다.
- 게임화면에는 설정, 가방, 스테이지, 몬스터 HP, 재화량, 보스 타이머, 하단탭이 있다.

<br>

## 2. 튜토리얼 및 스탯에 대한 설명

- 스탯 중 약점 공격 추가 및 몽환나비의 회피율을 추가할 지 고민중...................

- 게임화면이 실행되면 튜토리얼이 자동으로 실행된다.
- 이 튜토리얼은 게임의 기본적인 방식이나 스토리를 설명한다.
- 이 튜토리얼에 사용될 몬스터는 고블린이다.

- 일반피해량은 흰색 텍스트, 치명타피해량은 노란색 텍스트를 출력한다.
- 기본 치명타피해량은 일반피해량의 200% 다.
...
- 노말 스테이지 몬스터를 처치 시 매우 낮은 확률로 아이템을 드랍한다.
...
|:----:|:----:|:----:|:----:|
|전사|물리 공격|근접 공격|1명 공격|
|궁수|물리 공격|원거리 공격|1명 공격|
|공격형_마법사|마법공격|원거리 공격|다수 공격|
...
캐릭터의 종류|공격력|공격주기|치명타확률|치명타피해량|
|:----:|:----:|:----:|:----:|:----:|
전사|200|5초|0%|200%
궁수|150|4초|0%|200%
공격형_마법사|400|10초|0%|200%
지원형_마법사| 해당사항없음| 강화주기 30초, 정화주기 120초|해당사항없음|해당사항없음|

<br>

## 아이템의 종류 및 수치
- 폭탄의 종류(재사용 대기시간: 30초)
- 폭탄 시작 위치는 전사 캐릭터 기준이다.
- 몬스터의 위치는<br>


|폭탄 색상|사용 효과|지속시간|비고| 
|:----:|:----:|:----:|:----:|
|빨간색|몬스터의 체력 재생 0으로 변경한다.|15초간||
|주황색|몬스터의 받는 피해량 10% 증가한다.|10초간||
|노란색|몬스터를 기절 상태로 만든다.|3초간|모든 공격이 치명타로 들어간다, 비행 중인 몬스터는 추락한다.|
|초록색||||
|파란색|몬스터의 보호막 파괴한다.|||
|남색|몬스터에게 피해를 입힌다.||공격력에 비례한 피해를 입힌다.|
|보라색|몬스터을 부식시킨다.||부식은 경감 1단계를 내린다.|

<br>

- 물약1 종류(물약1, 물약2 중첩 사욥불가) <br>
...
- 철_골렘
- 기본적으로 경감이 3단계를 가지고 있으며

- 철골렘 설명
- 보호막 ㅂ주어
- 체력 50% 이하시 피해량 감소 보호막 부여
-
